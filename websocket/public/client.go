...
type PublicMessageType string

// 메시지 유형을 정의하는 상수들입니다.
const (
	MessageTypeTicker    PublicMessageType = "ticker"
	MessageTypeOrderbook PublicMessageType = "orderbook"
	MessageTypeTrade     PublicMessageType = "trade"
)

// Message는 웹소켓 메시지를 나타냅니다.
type Message struct {
	websocket.Message
	Type PublicMessageType `json:"type,omitempty"`
}
...
func AddSubscribe(messageType PublicMessageType, codes []string, options *common.SubscribeOptions) websocket.SubscribeFunc {
	if len(codes) == 0 {
